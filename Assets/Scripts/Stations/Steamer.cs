...
public class Steamer : Station {
    public Steamer(int number) { _number = number; }
    private int _number;
    ...
    */
    public override string[] Interact(string[] hands) {
        ...
        if(hands.Length == 0) {
            _module.log($"Took {slot.arrayToString()} out of the Steamer.");
            temp = slot;
            slot = new string[0];
            updateText();
            timer = 0;
        ...
        _module.log($"Put {hands.arrayToString()} in the Steamer.");
        timer = 0;
        burning = 0;
    ...
    public override void updoot() {
        ...
        if((timer >= 20 && !_module.TPStrikeTimer) || (timer >= 30 && _module.TPStrikeTimer)) {
            burning = 100;
            _module.Strike($"{slot.arrayToString()} burned in the Steamer.");
        }
