...
public class Fryer : Station {
    public Fryer(Overcooked module, int number) { _module = module; _number = number; }
    private Overcooked _module;
    private int _number;
    string[] uncooked = { "cutPotato", "cutChicken" };
    string[] cooked = { "friedPotato", "friedChicken" };
    public new string[] slot = new string[0];
    public new int Image = 4;
    public new string Color = "O";
    ...
    public override string[] Interact(string[] hands) {
        if(slot.Length == 0 && hands.Length == 1) {
            if(Array.IndexOf(uncooked, hands[0]) != -1) {
                ...
                _module.log($"Put {hands[0]} into the fryer.");
                return new string[0];
            }
            _module.log("Invalid Fryer Item.");
            return hands;
        }
        if(slot.Length == 0) {
            if(hands.Length == 0) {
                _module.log("Holding nothing and nothing in the fryer");
                return hands;
            }
            _module.log("Can only put one item in the fryer.");
            return hands;
        }
        ...
        _module.log($"Took {slot[0]} out of the fryer.");
    ...
    public override void updoot() {
        ...
        if(timer >= 20 && !_module.TPStrikeTimer) {
            burning = 100;
            _module.Strike($"{slot[0]} burned in fryer.");
        }
