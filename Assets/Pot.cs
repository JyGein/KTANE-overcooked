...
public class Pot : Station {
    public Pot(Overcooked module, int number) { _module = module; _number = number; }
    private Overcooked _module;
    private int _number;
    string[] uncooked = { "rice", "pasta" };
    ...
    public override string[] Interact(string[] hands) {
        if(slot.Length == 0 && hands.Length == 1) {
            if(Array.IndexOf(uncooked, hands[0]) != -1) {
                ...
                _module.log($"Put {hands[0]} into the pot.");
                return new string[0];
            }
            _module.log("Invalid Pot Item.");
            return hands;
        }
        if(slot.Length == 0) {
            if(hands.Length == 0) {
                _module.log("Holding nothing and nothing in the pot");
                return hands;
            }
            _module.log("Can only put one item in the pot.");
            return hands;
        }
        ...
        _module.log($"Took {slot[0]} out of the pot.");
    ...
    public override void updoot() {
        ...
        if(timer >= 20 && !_module.TPStrikeTimer) {
            burning = 100;
            _module.Strike($"{slot[0]} burned in Pot.");
        }
