...
public class Steamer : Station {
    ...
    */
    public override string[] Interact(string[] hands) {
        ...
        if(hands.Length == 0) {
            _module.log($"Took {_module.arrayToString(slot)} out of the Steamer.");
            temp = slot;
            slot = new string[0];
            updateText();
            timer = 0;
        ...
        _module.log($"Put {_module.arrayToString(hands)} in the Steamer.");
        timer = 0;
        burning = 0;
    ...
    public override void updoot() {
        ...
        if(timer >= 15 && burning == 0) {
            _module.log($"{_module.arrayToString(slot)} is done cooking");
        ...
        if((timer >= 20 && !_module.TPStrikeTimer) || (timer >= 30 && _module.TPStrikeTimer)) {
            burning = 100;
            _module.Strike($"{_module.arrayToString(slot)} burned in the Steamer.");
        }
